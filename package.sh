...
main() {
    ...
    local _version=$($_cargodir/target/release/inagent --version)
    local _pkgdir="inagent-$_version"
    ...
    # Project assets
    cp "$_cargodir/target/release/inagent" "$_pkgdir"
    sed "s~{{sysconfdir}}~$sysconfdir~" < "$_cargodir/resources/agent.json.tpl" > "$_pkgdir/agent.json"
    ...
    cp "$_cargodir/resources/init/systemd" "$_pkgdir/init"
